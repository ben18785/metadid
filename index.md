...
studies from a known hierarchical model.
[`as_summary_did()`](https://ben18785.github.io/metadid/reference/as_summary_did.md)
then aggregates this to the four-cell summary statistics (pre/post ×
control/treatment) that represent what is typically reported in a
published study.
...
fits a hierarchical Bayesian model. Setting `method = "optimize"` finds
the maximum a posteriori (MAP) estimate via L-BFGS — this is faster than
MCMC but provides no uncertainty measures in estimates.
...
of population- and study-level parameters. With MAP optimisation, only
point estimates are available (`sd`, `lo`, and `hi` are `NA`); use
`method = "sample"` for full posterior uncertainty.
...
posterior standard deviations and credible intervals:
...
![](reference/figures/README-pp-check-cdf-1.png)
...
![](reference/figures/README-pp-check-effects-1.png)
