...
#'    \eqn{(\mu_\theta, \gamma)} with the same product gives identical
...
#'    the model will still run but `gamma_mult` and `beta_cov[k]` will be
...
validate_multiplicative_covariate <- function(multiplicative_covariate_name,
  ...
  if (length(unique_vals) < 2) {
    ...
    stop(
      ...
      col, "', the multiplier gamma_mult and the population treatment ",
      "effect mu are jointly unidentified — any (mu, gamma) with the same ",
    ...
    for (cv in covariate_names) {
      ...
      if (!is.na(r) && abs(r) > threshold) {
        warning(
          ...
          " > ", threshold, "). gamma_mult and beta_cov[",
