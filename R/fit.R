...
print.meta_did_fit <- function(x, prob = 0.9, ...) {
  ...
  if (x$method == "sample") {
    ...
    if (!is.null(x$multiplicative_covariate)) {
      ...
      g_mean      <- mean(gamma_draws)
      g_lo        <- stats::quantile(gamma_draws, (1 - prob) / 2)
      g_hi        <- stats::quantile(gamma_draws, 1 - (1 - prob) / 2)
      cat(sprintf("Multiplicative covariate (%s): %.3f  %g%% CI [%.3f, %.3f]\n",
                  x$multiplicative_covariate, g_mean, prob * 100, g_lo, g_hi))
    }
  } else {
    mle <- x$fit$mle()
    ...
    if (!is.null(x$multiplicative_covariate)) {
      gamma_k <- mle[["gamma_mult[1]"]]
      cat(sprintf("Multiplicative covariate (%s): %.3f  (MAP estimate)\n",
                  x$multiplicative_covariate, gamma_k))
    }
  }
...
summary.meta_did_fit <- function(object, prob = 0.9, ...) {
  ...
  if (!is.null(object$multiplicative_covariate)) {
    ...
    if (!is.null(gamma_summary)) {
      gamma_summary$parameter <- paste0("gamma_mult[", object$multiplicative_covariate, "]")
