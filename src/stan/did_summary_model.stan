...
if(n_studies_did_summary > 0) {
  ...
  for (i in 1:n_studies_did_summary)
    mult_did_summary[i] = mult_factor(has_multiplicative_covariate, effect_multiplier, x_mult_did_summary[i]);
...
if (n_studies_did_change_only > 0) {
  ...
  if (is_student_t_heterogeneity) {
    ...
    for (i in 1:n_studies_did_change_only)
      mult_did_change_only[i] = mult_factor(has_multiplicative_covariate, effect_multiplier, x_mult_did_change_only[i]);
