...
vector[n_studies_rct] baseline_difference_rct;
...
if (!is_student_t_heterogeneity && !is_correlated_effects) {
  for (i in 1:size(treatment_effect_rct_raw))
    treatment_effect_rct[i] = overall_mult(effect_multiplier, x_mult_rct[i], effect_multiplier2, x_mult2_rct[i]) * (treatment_effect_mean_rct + X_cov_rct[i] * beta_cov) + treatment_effect_sd * treatment_effect_rct_raw[i];
} else {
...
// s_i is a transformed parameter here (see did_model_transformed_parameters).
// Post-only RCTs have no pre-treatment SDs, so the post-period ones stand in --
// the same substitution the summary path makes in R.
vector<lower=0>[n_studies_rct] gamma_scale_rct;
for (i in 1:n_studies_rct)
  gamma_scale_rct[i] = sqrt(
    square(sigma_treatment_after_rct[i]) / sample_size_treatment_rct[i] +
    square(sigma_control_after_rct[i])   / sample_size_control_rct[i]);

for (i in 1:size(baseline_difference_rct_raw))
  baseline_difference_rct[i] = gamma_from_raw(
    gamma_mode_rct[i], baseline_difference_rct_raw[i],
    baseline_difference_mean, baseline_difference_sd,
    kappa, gamma_scale_rct[i]);
