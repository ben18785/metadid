...
// but the canonical→absolute multiplication of θ_T and β_T does. In the
...
// sigma_treatment_before_pp is needed in the bivariate path AND in the
// differenced+modelled path, where it anchors the per-study latent
// baseline via an explicit Gaussian likelihood on the individual pre
// observations. Sized accordingly.
vector<lower=0>[n_studies_pp * (1 - is_differenced_likelihood_pp + is_differenced_likelihood_pp * is_modelled)] sigma_treatment_before_pp;
