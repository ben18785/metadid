...
if (!is_student_t_heterogeneity && !is_correlated_effects) {
  for (i in 1:n_studies_pp)
    treatment_effect_pp[i] = mult_factor(has_multiplicative_covariate, effect_multiplier, x_mult_pp[i]) * (treatment_effect_mean_pp + X_cov_pp[i] * beta_cov) + treatment_effect_sd * treatment_effect_pp_raw[i];
} else {
