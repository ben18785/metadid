...
metadid supports this through the `multiplicative_covariate` argument. The covariate is categorical with a reference level: one `multiplier` is estimated per non-reference level and applied to the studies at that level, while the reference level's factor is fixed at 1. A binary indicator $m_i \in \{0, 1\}$ is the two-level special case, with a single estimated `multiplier` applied to the studies where it equals 1:
...
Studies with $m_i = 0$ identify the linear predictor directly; studies with $m_i = 1$ see it multiplied by the `multiplier`. The multiplier is constrained to be non-negative, with a `normal(1, 0.5)` prior centred at 1 — the no-effect case where every study contributes to the same population mean.

### Example: experimental vs real-world studies

```{r mult-example, eval = FALSE}
# Mark each study with a binary indicator
...
#> Population treatment effect: -0.412  90% CI [-0.450, -0.376]
#> Multiplicative covariate (real_world): 0.51  90% CI [0.42, 0.61]
```

The interpretation: experimental studies produce an effect around -0.412, while real-world studies produce roughly half of that. The combination yields the per-study mean used in the meta-analysis.
...
The covariate is not restricted to a binary indicator. With a column such as
...
summary(fit)
# rows: effect_multiplier[rw_lab], effect_multiplier[rw_field]
```
...
input the levels sort in ascending order (so a `{0, 1}` indicator behaves
exactly as before), and for character input alphabetically. The same
`multiplier` prior from `set_priors()` is applied independently to each
estimated factor, and the data must contain studies at two or more levels for
the multipliers to be identified.
