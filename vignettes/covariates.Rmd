...
metadid supports this through the `multiplicative_covariate` argument. With a binary indicator $m_i \in \{0, 1\}$ and a single multiplier $\gamma_{\mathrm{mult}}$, the per-study mean becomes

$$\mu_i = \gamma_{\mathrm{mult}}^{m_i} \cdot \bigl(\mu_\theta + X_{\mathrm{cov},i}^{\top}\beta_{\mathrm{cov}}\bigr).$$

Studies with $m_i = 0$ identify $\mu_\theta + X_{\mathrm{cov},i}^{\top}\beta_{\mathrm{cov}}$ directly; studies with $m_i = 1$ see that linear predictor multiplied by $\gamma_{\mathrm{mult}}$. The multiplier is constrained to be non-negative by default (`set_priors(multiplier = normal(1, 0.5, lower = 0))`), and is centred at 1 — the no-effect case where every study contributes to the same population mean.
...
* **Hard error: constant indicator.** If every study has $m_i = 0$ or every study has $m_i = 1$, then $\mu_\theta$ and $\gamma_{\mathrm{mult}}$ are jointly unidentified — any pair $(\mu_\theta, \gamma_{\mathrm{mult}})$ with the same product gives identical likelihood. `meta_did()` stops with an error pointing to the column.
